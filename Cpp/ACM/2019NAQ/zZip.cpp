...
double w, a, b, r;
double f(double mid) { return (dist(0, a, mid, 0) + dist(w, b, mid, 0)); }

double d(double m) {
  return m / sqrt(m * m + a * a) - (w - m) / sqrt(b * b + (w - m) * (w - m));
}

double maxDis(double a, double b) {
  if (b - a < EPS) {
    return a;
  }
  double m = MID(a, b);
  // db(a, b, d(m));
  if (d(m) < 0) return maxDis(m, b);
  return maxDis(a, m);
}
...
int main() {
  ...
  for (l i = 0; i < n; i++) {
    ...
    a -= r;
    b -= r;
    dc(8) dist(0, a, w, b) << " " << f(maxDis(0, w)) << "\n";
  }
}
