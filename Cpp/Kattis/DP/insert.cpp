...
class Node {
    ...
    bigint visit(){
        if(!this->count.isZero()) return this->count;
        if(this->value < 0) return bigint(1);

        // std::cerr "VIS: " << this->a->visit() << " " << this->b->visit() << " " << "\n";
        this->count = dp[this->a->children][this->b->children] * (this->a->visit()) * (this->b->visit());
        return this->count;
    }
